...
if (authFromData !== undefined && authFromData !== "") {
    ...
    // auth_header rỗng → set empty để request không có Authorization
    pm.variables.set("currentAuthHeader", "");
}
```

> **Lưu ý:** Trong Postman, nếu `{{currentAuthHeader}}` rỗng, header vẫn được gửi với giá trị rỗng. Để **bỏ hoàn toàn** header cho negative case, cần dùng Pre-request Script set header động (xem mục 2.1).

### 2.1 Gắn header động bằng `pm.request.headers`

```javascript
// Pre-request Script — Gắn/xóa Authorization header theo iteration data
const authValue = pm.variables.get("currentAuthHeader");

if (!authValue || authValue === "") {
    // Xóa header Authorization — giả lập "thiếu token"
...
const expectArray   = pm.iterationData.get("expect_array") === true
...
if (expectArray) {
    pm.test(`[${tcId}] Response là array với ít nhất ${expectMinLen} phần tử`, () => {
...
const desc           = pm.iterationData.get("description") || "";
const expectedStatus = parseInt(pm.iterationData.get("expected_status"));
const expectHasId    = pm.iterationData.get("expect_has_id") === true
...
const desc           = pm.iterationData.get("description") || "";
const expectedStatus = parseInt(pm.iterationData.get("expected_status"));
const expectEmpty    = pm.iterationData.get("expect_empty_body") === true
